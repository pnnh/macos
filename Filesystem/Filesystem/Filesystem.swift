...
struct FilesystemView: View {
    @State var targetDir: String? = "/Users/Larry/Pictures/album"

    var body: some View {
        VStack {
            Button("点击第一个按钮选择目录") {
                do {
                    let dir = try FilesystemHandler.chooseDirectory()
                    print("dir \(dir ?? "nil")")
                    targetDir = dir
                } catch {
                    print("err: \(error)")
            ...
            Button("点击第二个按钮遍历目录") {
                if targetDir != nil {
                    do {
                        let list = try FilesystemHandler.scanDirectory(path: targetDir!, filter: "")
...
public class FilesystemHandler {
    ...
    public static func scanDirectory(url: URL, filter: String) throws -> [String] {
        ...
        let contents: [URL] = try fileManager.contentsOfDirectory(
            ...
            options: [FileManager.DirectoryEnumerationOptions.skipsHiddenFiles, FileManager.DirectoryEnumerationOptions.skipsSubdirectoryDescendants]
        )
